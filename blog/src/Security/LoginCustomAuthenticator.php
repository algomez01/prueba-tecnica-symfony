...
class LoginCustomAuthenticator extends AbstractLoginFormAuthenticator
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token, string $firewallName): ?Response
    {
        ...
        if($this->auth->isGranted(User::ROLE_ADMIN)){
            return new RedirectResponse($this->urlGenerator->generate('app_categoria_index'));
        }elseif($this->auth->isGranted(User:: ROLE_SUPERVISOR)){
            return new RedirectResponse($this->urlGenerator->generate('app_publicaciones_index1'));
        ...
        }elseif($this->auth->isGranted(User::ROLE_PERSONAS)){
            return new RedirectResponse($this->urlGenerator->generate('app_publicaciones_index'));
        }
